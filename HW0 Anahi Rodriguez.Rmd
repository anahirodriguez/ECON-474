...
***************************************************************************************
| Homework || Exams || Final Project ||
| --- |--- | --- | --- |---|---|
| 0: R Markdown Basics (R)                 |10% | RE & IV   |20% | Replication | 10% |
| 1: Randomized Experiments (RE)           |10% | RDD & DiD |20% |  ||
| 2: Instrumental Variables (IV)           |10% | |   |||
| 3: Regression Discontinuity Design (RDD) |10% ||   |||
| 4: Differences-in-Differences (DiD)      |10% | |   |||
| **Total:**                               |50% |           |40% |             | 10% |
***************************************************************************************
...
**[5 Points]** Print your hypothetical final grade.
...
hw1_grade = (39/46)
hw2_grade = (73/80)
hw3_grade = (16743/18110)
hw4_grade = (9/11)
exam1_grade = (18/23)
exam2_grade =  (2/2)
final_project_grade = (89/100)
piazza_grade = (2)
recording_reminder_grade =  0
sum((hw0 * hw0_grade),
    (hw1 * hw1_grade),
    (hw2 * hw2_grade),
    (hw3 * hw3_grade),
    ...
    (exam1 * exam1_grade),
    (exam2 * exam2_grade),
    (final_project * final_project_grade)) + (piazza * piazza_grade) + (recording_reminder * recording_reminder_grade)
...
**[4 Points]** What percentage of points **currently available** have you received so far?
...
(sum((hw0 * hw0_grade),
    ...
    (hw4 * hw4_grade),
    (exam1 * exam1_grade),
    (exam2 * exam2_grade)) + (piazza * piazza_grade) )
...
(.83 - (sum((hw0 * hw0_grade),
    (hw1 * hw1_grade),
    (hw2 * hw2_grade),
    (hw3 * hw3_grade),
    (hw4 * hw4_grade),
    (exam1 * exam1_grade),
    (exam2 * exam2_grade)) + (piazza * piazza_grade)))/0.1
...
$$log(y_i) = \beta_0 + log(x_i)\beta_1 + \epsilon_i$$
...
$$
\begin{align}
  \frac{dlog(y_i)}{dlog(x_i)} &= \beta_1\\
  \frac{\frac{dy_i}{y_i}}{\frac{dx_i}{x_i}} &= \beta_1\\
  \beta_1 &\approx \frac{\frac{\Delta y}{y}}{\frac{\Delta x}{x}}\\
...
```{r bayes1}
# Solution R
212/712  == ((212/250)*(250/50000))/(712/50000)
```
...
```{r bayes2}
# Solution R
712/50000 == ((212/250)*(250/50000)) + ((500/49750)*(49750/50000))
```
...
```{r bayes3}
# Solution R
a = (((500/49750)^2)*(49750/50000))/((((500/49750)^2)*(49750/50000)) + ((212/250)^2)*(250/50000))
a
```
...
df = read.csv("mhe_3_2_1.csv")

label = unique(c(df$HGC.FATHER_1979, df$HGC.MOTHER_1979, df$HGC_2002))
value = c(8, 12, 6, 10,
          16, 15, 18, 17,
          20, 4, 3, 9,
          14, 5, NA, 11,
...
mapping = data.frame(label,value)


library(tidyverse)
df = df %>%
  filter(SAMPLE_SEX_1979 == 'MALE' & Q13.5_TRUNC_2002 > 0) %>%
  left_join(mapping, by = c("HGC.MOTHER_1979" = "label")) %>%
  rename(mother_yos = value) %>%
  left_join(mapping, by = c("HGC.FATHER_1979" = "label")) %>%
  rename(father_yos = value) %>%
  left_join(mapping, by = c("HGC_2002" = "label")) %>%
  rename(yos = value) %>%
  mutate(race = as.factor(SAMPLE_RACE_78SCRN),
         age = as.factor(AGEATINT_2002),
         occ = as.factor(OCCALL.EMP.01_2002),
         AFQT.3_1981 = AFQT.3_1981/1000) %>%
  na.omit()
...
```{r regrssions}
fit1 = lm(log(Q13.5_TRUNC_2002) ~ yos, data = df, weight = SAMPWEIGHT_2002)
fit2 = lm(log(Q13.5_TRUNC_2002) ~ yos + age, data = df, weight = SAMPWEIGHT_2002)
fit3 = lm(log(Q13.5_TRUNC_2002) ~ yos + age + mother_yos + father_yos + race + REGION_2002, data = df, weight = SAMPWEIGHT_2002)
fit4 = lm(log(Q13.5_TRUNC_2002) ~ yos + age + mother_yos + father_yos + race + REGION_2002 + AFQT.3_1981, data = df, weight = SAMPWEIGHT_2002)
fit5 = lm(log(Q13.5_TRUNC_2002) ~ yos + age + mother_yos + father_yos + race + REGION_2002 + AFQT.3_1981 + occ, data = df, weight = SAMPWEIGHT_2002)
...
```{r fwl}
fit4_x = lm(yos ~ age + mother_yos + father_yos + race + REGION_2002 + AFQT.3_1981, data = df, weights = SAMPWEIGHT_2002)
x_tilde = fit4_x$residuals
df$x_tilde_yos = x_tilde

ggplot(data = df, aes(x = x_tilde_yos, y = log(Q13.5_TRUNC_2002))) + 
  geom_point() + 
  geom_smooth(method = 'lm') + 
  labs(x = 'Frisch-Waugh-Lovell Residuals',
       y = 'Log Wages',
       title = 'Frisch-Waugh-Lovell Plot')
```
...
stargazer(fit1, fit2, fit3, fit4, fit5,
          ...
          keep  = c('yos', 'mother_yos', 'father_yos','AFQT.3_1981'),
          covariate.labels = c('Years of Schooling', "Mother's Years of Schooling",
                               "Father's Years of Schooling", 'AFQT Score'),
          dep.var.label = c('Log Wages'),
          add.lines = (list(c('Age', 'No', rep('Yes', 4)),
                         c("Race", rep('No',2), rep('Yes',3)),
                         c('Census Region', rep('No',2), rep('Yes', 3)),
                         c('Occupation', rep('No', 4), rep('Yes', 1)))),
          star.char = c('+','*', '**', '***'),
...
The occupation variable is a bad control variable because it is a collider, occupation is determined after sex which opens a backdoor path that will bias results.
