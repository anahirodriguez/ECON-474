...
**Your answer:**
The parallel trends assumption: in the absence of treatment, the difference in the outcome of the treatment and the control group would be constant
...
**Your answer:**
No, because we do not observe the counterfactual of the treatment group not being treated
...
**Your answer:**
No, they indirectly test the pre treatment parallel trends. This is an indirect test because it could just be a coincidence and becuase it only tests the first half of the assumtion.
...
df = read_csv("HW DiD data_person_4.csv")
```
...
```{r fit_tab2}
fit_tab2 = feols(c(ln_y, ln_yrel) ~ ln_ya_w_g2*d2 + urban + schooling + age +
                   electricity + water + isfemale + percepho + mieperho | 
                   ubigeo + year + ciuu_cod1 + p507,
                   data = subset(df, ocu500 == 1 & codperso ==1), 
                   cluster = 'ubigeo',
                   weights = ~factor)
...
```{r tab_dict1}
tab_dict1 = c(ln_y = 'Nominal Income',
              ln_yrel = 'Real Income',
              ln_ya_w_g2 = 'Local Input Demand',
              d2 = 'Distance < 100 km',
              ubigeo = 'District',
...
```{r table 2}
etable(fit_tab2, se.below = T, dict = tab_dict1, 
       keep = 'Local Input Demand x Distance < 100 km',
       fitstat = c('n', 'r2'))
...
```{r figure reg}
fit_fig4 = feols(c(ln_y, ln_yrel) ~ ln_ya_w_g2*factor(proxr) + urban + schooling + age +
                   electricity + water + isfemale + percepho + mieperho | 
                   ubigeo + year + ciuu_cod1 + p507,
                   data = subset(df, ocu500 == 1 & codperso ==1), 
                   cluster = 'ubigeo',
...
```{r coef selection}
df_event = bind_cols(coef(fit_fig4[[2]]), confint(fit_fig4[[2]], level = 0.9, cluster = ~ ubigeo))

df_event = df_event %>%
  filter(str_detect(row.names(df_event), ':')) %>%
  mutate(dist = factor(c("200 km", "150 km", "100 km", "50 km", "City")),
         dist = fct_reorder(dist, rev(levels(dist))))
...
```{r event figure}
ggplot(df_event, aes(x = dist, y = `...1`, ymin = `5 %` , ymax = `95 %` )) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  labs(title = str_wrap("Effect of Yanachocha's Expansion on Real Income, By Distance to Cajamarca City", 60), x = 'Distance (km)', y = 'Size of the Effect' , caption = '90% confidence intervals') +
  geom_errorbar()
...
```{r ocu500 na}
df = df %>%
  mutate(ocu500 = ifelse(is.na(ocu500), -9, ocu500))
```
...
```{r migration}
feols(c(schooling, prim_full, prime) ~ ln_ya_w_g2*d2 | ubigeo + year,
      weights = ~ factor,
      data = df,
      subset = ~ ocu500 != 4) %>%
  etable(dict = tab_dict2, se.below = T, 
       keep = 'Local Input Demand x Distance < 100 km',
       fitstat = c('n', 'r2'))
