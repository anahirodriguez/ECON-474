...
# original cleaning of data given to us & not running this chunk becuase just loading in much smaller clearer data
...
ss = usa %>%
  ...
  filter(empstat == 1) %>% # 3veryone who is employed
  filter(age >= 18 & age <= 65 &#working age population
           gq %in% c(1,2) & #not in group quarters
           workedyr == 3 & #those who worked last year
           incwage > 0 & incwage < 999998) %>% #define wages
  filter(ind1990 > 0 & ind1990 < 900) %>% # defined industries and not a gov/military worker
  # not every MSA is in every year of data
  # we want a consistent panel of MSAs,
  # so we are going to select those that are present in all 4 years
  group_by(metarea) %>% 
  # identify those in all 4 years by counting unique years
  mutate(n_metarea = n_distinct(year)) %>%
  # filter to only have MSAs in all four years
  filter(n_metarea == 4) %>%
...
ss = fread("shift_shares cleaned.csv")
...
emp_agg = ss %>%
  filter(ind1990 %in% c(10,11:392, 500:711, 721, 732, 800, 832, 882:892)) %>% # tradeable industried affected by product demand
...
for(a in unique(emp_agg$metarea)) {
  ...
  temp = temp %>%
    ...
    summarise(emp_atilda_jt = sum(emp_ajt),
              .groups = 'drop') %>%
...
temp_t_shift = emp_t_shift %>%
  arrange(metarea, ind1990, year) %>%
  group_by(metarea, ind1990) %>%
  mutate(emp_atilda_jt_lag = lag(emp_atilda_jt),
         shift = log(emp_atilda_jt) - log(emp_atilda_jt_lag)) %>%
...
emp_t0_share = emp_agg %>%
  ...
  mutate(emp_njt = sum(emp_ajt),
         share = emp_ajt/emp_njt) %>% # local area over national
...
etable(fit_ols, fit_rf, fit_fs, fit_2sls,
       se.below = T, 
       fitstat = c('n', 'r2', 'f'),
       headers = c('OLS', " Reduced Form", "First State", "2SLS")
       
       
# large f-stat confirms the exclusion restriction is met       
...
# need a strong estimate for first stage or else hige number on either side of infinity
  # this is confirmed by the large f - stat that we saw in the first stage
