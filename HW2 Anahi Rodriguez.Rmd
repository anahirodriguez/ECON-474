...
```{r data}
df = fread("~/Desktop/data econ 474/usa_00001.csv")

names(df) = names(df) %>% tolower

df = df %>%
  select(c(year, region, bpl, metro, sex, age, birthqtr, birthyr, 
           marst, race, educ, wkswork1, incwage, gq, educd)) # check the variables used here
...
dim(df %>%
  filter(sex == 1,
         ...
         bpl %in% c(1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 15:42, 44:51, 53:56),
         gq == 1,
         wkswork1 >= 1,
         educd != 1) %>%
  mutate(wkly_wage = incwage/wkswork1,
         ...
         cental_msa = ifelse(metro %in% c(2,4), 1, 0),
         birthdate = case_when(birthqtr == 1 ~ paste(birthyr),
                               birthqtr == 2 ~ paste(birthyr, '.25'),
                               birthqtr == 3 ~ paste(birthyr, '.5'),
                               birthqtr == 4 ~ paste(birthyr, '.75')),
