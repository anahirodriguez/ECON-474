...
We are creating the control group by estimating **propensity scores of being treated** using data with observations that are treated and untreaded via logistic regression.
...
```{r loading data}
path_data = "~/Desktop/data econ 474/" # path to our data folder
...
if(!file.exists(paste0(path_data, file_cps))) {
  ...
  write_dta(paste0(path_data, file_cps))
} else {
...
for(v in names(df)[!names(df) %in% c('data_id', 'treat')]){
  bal_tab$pvalue[bal_tab$variable == v] = t.test(df[[v]] ~ df[['treat']])$p.value
}
```
