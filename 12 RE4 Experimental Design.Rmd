...
# will use this to find the power of statistical tests - the likelihood of correctly rejecting a null
```

However, we are going to focus on the two t-test functions

1. `pwr.t.test()`: same control and treatment group size
2. `pwr.t.test()`: differing control and treatment sizes
...
df = read.csv("Thornton 2008 main sample.csv")
...
```{r post effect size}
fit = lm(data = df, got ~ incentive-1)
summary(fit)
b = coef(fit) # null hypothesis that incentive coefficeint is different than 0 
b[1]
sd = sd(df$any)
eff_size = as.numeric((b[1] - b[2])/sd)
eff_size
