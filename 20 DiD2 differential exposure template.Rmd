...
df = read.csv("DiD2 differential exposure.csv")
...
fit_event = feols(q90_q10_tot ~ year_factor * log(comp_90) + log(pop_90) +
                    ba_share_90 + log(med_house_value_90), 
                  data = df, se = 'hetero')
...
df_event = bind_cols(coef(fit_event), confint(fit_event, se = 'hetero')) %>%
  rename(delta_k = ...1)
...
ggplot(df_event, aes(x = year, y = delta_k, ymin = `2.5 %`, ymax = `97.5 %`)) +
  ...
  geom_rect(xmin = 1995.25, xmax = 2000.25, 
            ymin = -Inf, ymax = Inf, alpha = 0.05, 
            aes(fill = "Dotcom\nBubble")) +
  scale_fill_manual(values = 'black') +
  labs(x = 'Year', y = TeX('$\\delta_k$'), title = '90-10 Total Inocme Ratio',
       fill = '') +
  theme_minimal() +
  theme(text = element_text(size = 20)) # must go after appearance themes
```
...
fit_main = feols(q90_q10_tot ~ post * log(comp_90) + log(pop_90) +
                    ba_share_90 + log(med_house_value_90), 
                  data = df_reg,  se = 'hetero')
etable(fit_main, se.below = T)
```
...
```{r finance placebo}
feols(q90_q10_tot ~ post * log(fin_share_90) + log(pop_90) +
                    ba_share_90 + log(med_house_value_90), 
                  data = df_reg,  se = 'hetero') %>%
  etable()
```
...
```{r no way San Jose}
feols(q90_q10_tot ~ post * log(comp_90) + log(pop_90) +
                    ba_share_90 + log(med_house_value_90), 
                  data = df_reg,  se = 'hetero',
      subset = ~metarea != 740) %>%
  etable()
...
feols(q90_q10_tot_lag ~ post * log(comp_90) + log(pop_90) +
                    ba_share_90 + log(med_house_value_90), 
                  data = df_reg_lag,  se = 'hetero') %>%
  etable() 
```
