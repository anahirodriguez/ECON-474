...
cps = mutate(cps,
       married = (marst <= 3)*1, # logic statement
       sex = ifelse(sex == 1, 'male', 'female'), # ifelse() male == 1
       ...
       empstat = case_when(empstat %in% c(10, 12) ~ 'employed',
                           ...
                           TRUE ~ 'NILF'),
       earnweek = case_when(earnweek < 9999.99 ~ earnweek)) # fixing bad NAs 
head(cps)
```
...
ym(paste(cps$year, '-', cps$month))

cps = mutate(cps, date = ym(paste(cps$year, '-', cps$month)))
class(cps$date)
```
...
summarise(cps, 
          U = sum(wtfinl*(empstat == 'unemployed')), 
...
ggplot(urate, aes(x = date, y = urate )) +
  geom_line() + 
  geom_point() + # size to change size and alpha to change transparency level
  labs(y = 'Unemployment Rate (%)', x = 'Month',
