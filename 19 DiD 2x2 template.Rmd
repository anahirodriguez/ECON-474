...
banks = read.csv( "DiD1 Mississippi experiment.csv")
...
```{r parllel trends df}
df_plot = banks %>%
  mutate(diff_d8_d6 = X8th.district[year == 1930] - X6th.district[year == 1930],
         counterfactual = ifelse(year >= 1930,
                                 X8th.district - diff_d8_d6,
                                 X6th.district)) %>%
  select(-diff_d8_d6) %>%
  pivot_longer(cols = c(X8th.district, X6th.district, counterfactual),
               names_to = 'district',
               values_to = 'n_banks')
...
# dahsed line must be the case for this effect to be casual
```
...
df_reg = df_plot %>%
  ...
  mutate(treat = (district == 'x6th.district') * 1,
         post = (year > 1930) * 1)
df_reg
```
...
```{r 2x2}
fit1 = feols(n_banks ~ treat * post, data = df_reg, se = 'hetero')
fit2 = feols(n_banks ~ treat * post | district, data = df_reg) # fixed effecgt of district
...
ggplot(df_reg, aes(x = year, y = n_banks, color = district)) +
  geom_line() + 
  labs(x = 'Year', y = 'Number of Banks in Business') +
  geom_smooth(method = 'lm', se = F, formula = y ~ I(x > 1930), linetype = 'dashed')
```
