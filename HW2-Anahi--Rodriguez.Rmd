...
figure_1 = 
  ...
  theme(plot.title = element_text(vjust = -100, hjust = 0.5),
        plot.subtitle = element_text(vjust = -125, hjust = 0.5),
...
I do believe that the intuition that using quarter of birth as an instrument for years of schooling is seen in these figures because we see that those born in quarter 1 follow the trend overall but tend to be lower values than the other 3 quarters for every year. This implies that we have a strong first stage which is used to test the inclusion restriction test. 
...
Yes, mean log weekly earnings is still lower for those born in quarter 1 compared to those born in quarters 2,3, and 4 for the same year accross all 20 years.
...
# not sure if should be below to be able to keep pbl later
table_3 = df %>%
  filter(birthyr %in% 1930:1939) %>%
  group_by(birthqtr, birthyr, birthdate) %>%
  mutate(m_l_wkly_wage = mean(log(wkly_wage)),
          m_yos = mean(yos, , na.rm = TRUE),
          iv1 = ifelse(birthqtr == 1, 'q1', 'not q1'))
...
y_z1 = mean(table_3$m_l_wkly_wage[table_3$iv1 == 'q1'])
y_z1


y_z0 = mean(table_3$m_l_wkly_wage[table_3$iv1 == 'not q1'])
y_z0
...
t.test(table_3$m_l_wkly_wage ~ table_3$iv1)$stderr


summary(lm(data = table_3, m_l_wkly_wage ~ iv1))
...
d_z1 = mean(table_3$m_yos[table_3$iv1 == 'q1'])
d_z1

d_z0 = mean(table_3$m_yos[table_3$iv1 == 'not q1'])
d_z0
...
t.test(table_3$m_yos ~ table_3$iv1)$stderr
summary(lm(m_yos ~ iv1, data = table_3))
...
table_5 %>%
  ungroup() %>%
  summarise(across(c(iv1, iv2, iv3, iv4), n_distinct))
...
```{r table 5 ish}
etable(fit_rf, fit_2sls1, fit_2sls2, fit_2sls3, fit_2sls4,
       se.below = T,
       stage = 1:2,
       keep = c('yos', 'black', 'married', 'central_msa'),
       order = 'yos',
       fitstat = c('n', 'r2', 'f', 'sargan', 'sargan.p'))
...
The instrumental variable does not correct the OLS bias that I predicted in subsection 1.1 because we see larger coefficients for years of schooling in the 2sls output than for the OLS output. 
This could result from a certain demographic, such as those who have the lowest level of ability, having a larger benefit/gain in wages from gaining more schooling.
...
The 2sls estimates shrink as we add more instruments
Sargan tells us we reject the null that the over identifying restrictions are valid; all instruments are valid as we continue to add more and more.
