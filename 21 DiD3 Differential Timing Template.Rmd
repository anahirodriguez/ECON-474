...
castle = read_csv("DiD3 castle.csv")
...
```{r event df prep}
df = castle %>%
  filter(treated == 1 & cdl < 1) %>%
  group_by(state) %>%
  summarise(reform_year = tail(year, 1)) %>%
  full_join(castle) %>%
  mutate(event_time = year - reform_year,
...
df_event = bind_cols(coef(fit_event), confint(fit_event, cluster = 'state')) %>%
...
```{r event plot}
ggplot(df_event, aes(x = event_time, y = `...1`, 
                     ymin = `2.5 %`, ymax = `97.5 %`)) +
  geom_point() +
  geom_line() + 
...
df %>%
  filter(treated == 1) %>%
  mutate(event_time = fct_relevel(event_time, '0', after = 9)) %>% # moves the event_time 0 factor to the 10th positioned bar
  ggplot(aes(x = factor(event_time))) + 
  geom_bar()
```
...
fit1_2 = feols(l_homicide ~ cdl | csw(state + year ,region:year),
               data = castle, weights = ~ popwt)

fit3 =  feols(l_homicide ~ cdl + l_police + unemployrt +
               poverty + l_income + l_prisoner + l_exp_subsidy + l_exp_pubwelfare +
               blackm_15_24 + whitem_15_24 + blackm_25_44 + whitem_25_44,
               data = df, weights = castle$popwt)

etable(fit1_2, fit3,
       keep = 'cdl',
       se.below = T,
       extralines = list('_Time-Varying Controls' = c('No', 'No', 'Yes' ))) 
       # ^ = top section (coefs)
       # - = middle section (fixed effects)
       # _ bottom section (fitstats)
```
...
```{r robustness}
feols(c(l_larceny, l_motor, l_robbery, l_assault) %>%
  etable(keep = 'cdl')
```
