...
hie = read_dta("oregonhie_data_for_analysis.dta")
...
# Generalized Regular Rxpression Logical Function
  # want to return a pattern 

# lll is for lottery numbers
# nnn is for the number of people in a household
...
# remidner: ddd is survey data and lll/nnn are administrative data
outcomes_hdd = c('ohp_all_ever_admin', 'ohp_std_ever_admin', 
                 'ohp_all_mo_admin', 'ohp_all_end_admin')
outcomes_12m = c('ohp_all_ever_survey', 'ohp_std_ever_survey', 
                 'ohp_all_mo_survey', 'ohp_all_end_survey')
...
i = 1
feols(formula(paste0(outcomes_hdd[i], '~', treatment, '+', controls_hdd, collapse = ' + ')),
      data = hie,
...
for(i in 1:length(outcomes_hdd)) {
  fs_fits_hdd[[i]] = feols(formula(paste0(outcomes_hdd[i], '~', treatment, '+', controls_hdd, collapse = ' + ')),
      data = hie,
      cluster = 'household_id')
...
i = 1
feols(formula(paste0(outcomes_12m[i], '~', treatment, '+', controls_12m, collapse = ' + ')),
      data = hie,
      cluster = 'household_id')
...
mean(pull(hie[row_selection, outcomes_12m[i]]))


for(i in 1:length(outcomes_12m)) {
  fs_fits_12m[[i]] = feols(formula(paste0(outcomes_12m[i], '~', treatment, '+', controls_12m, collapse = ' + ')),
                           data = hie,
                           cluster = 'household_id',
...
# large F-statistis are good for first stage
# positive treatment is telling us that those who are treated are actually becoming treated
...
```{r tab5}
tab5_y_12m = c('rx_any_12m','rx_num_mod_12m', 
...
for(i in 1:length(tab5_y_12m)) {
  ...
  # ITT - reduced form
  tab5[[counter]] = 
    feols(formula(paste0(tab5_y_12m[i], '~', treatment, '+', controls_12m, collapse = ' + ')),
                           data = hie,
                           cluster = 'household_id',
                           subset = hie$sample_12m_resp == 1,
                           weights = hie$weight_12m)
  ...
  fs_control_means_12m[counter] = 
    feols(formula(paste0(tab5_y_12m[i], '~', treatment, '+', controls_12m, "|ohp_all_ever_survey ~ treatment")),
                           data = hie,
                           cluster = 'household_id',
                           subset = hie$sample_12m_resp == 1,
                           weights = hie$weight_12m)
...
etable(tab5[[1]], tab5[[2]], tab5[[3]], tab5[[4]], 
       ...
       se.below = T,
       fitstat = c('n', 'r2'),
       keep = c('treatment', 'ohp_all_ever'),
       extralines = list('control means' = tab5_ctrl_m),
       headers = c('ITT', 'TOT'))
```
...
```{r tab9}
tab9_y_12m = c('health_genflip_bin_12m', 'notbaddays_phys_12m',
               'otbaddays_ment_12m')
...
for(i in 1:length(tab9_y_12m)) {
  ...
  # ITT - reduced form
  tab9[[counter]] = 
    feols(formula(paste0(tab9_y_12m[counter], '~', treatment, '+', controls_12m, collapse = ' + ')),
                           data = hie,
                           cluster = 'household_id',
                           subset = hie$sample_12m_resp == 1,
                           weights = hie$weight_12m)
  ...
  # TOT - structural equation
  fs_control_means_12m[counter] = 
    feols(formula(paste0(tab9_y_12m[i], '~', treatment, '+', controls_12m, "|ohp_all_ever_survey ~ treatment")),
                           data = hie,
                           cluster = 'household_id',
                           subset = hie$sample_12m_resp == 1,
                           weights = hie$weight_12m)
...
etable(tab9[[1]], tab9[[2]], tab9[[3]], tab9[[4]], 
       ...
       fitstat = c('n', 'r2'),
       keep = c('treatment', 'ohp_all_ever'),
       extralines = list('control means' = tab9_ctrl_m),
       headers = c('ITT', 'TOT'))
