...
fgroupsubtitle: "ECON 474 Econometrics of Policy Evaluation"
output: 
  html_document:
    toc: yes
    toc_float: true
...
reduced_form = feols(data = card, 
            lwage ~ nearc4 + exper + expersq + black + south + smsa,
            se = 'hetero')


# F-stat is large => exclusion restriction satisfied
first_stage = feols(data = card, 
            educ ~ nearc4 + exper + expersq + black + south + smsa,
            se = 'hetero')

fit_2sls = feols(data = card,
                            lwage ~ exper + expersq + black + south + smsa | educ ~ nearc4,
                            se = 'hetero')

etable(ols, reduced_form, first_stage, fit_2sls,
       se.below = T,
       fitstat = c('n', 'f', 'r2'),
...
the treatment affect would be larger for individuals who are poorer or others who benefit the most from going to college